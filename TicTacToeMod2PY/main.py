...
def make_move_player(field,char):
    cols = ["a","b","c"]
    rows = ["1","2","3"]
    while True:
...
def check_win(field):
    for i in range (1,4):
        if ((field[i][1]==field[i][2]==field[i][3]) and field[i][1]!="-") or ((field[1][i]==field[2][i]==field[3][i] and field[1][i]!="-")):

            return True
        if ((field[1][1]==field[2][2]==field[3][3]) or (field[3][1]==field[2][2]==field[1][3])) and field[2][2]!="-":
            return True
    return False
